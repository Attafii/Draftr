  ...
  return (
    <div id="magic-upload-zone" className={cn("relative w-full max-w-3xl p-px", className)}>
      ...
      <button
        ...
        aria-busy={isBusy}
        className={cn(
          "relative flex w-full flex-col overflow-hidden border border-white/10 bg-zinc-950/95 px-6 py-6 text-left shadow-shell outline-none transition-transform duration-300 focus-visible:ring-1 focus-visible:ring-white/40 sm:px-8 sm:py-8",
      ...
      >
        ...
        <div className="relative flex min-h-[420px] flex-col items-center justify-center text-center">
          <motion.div
            ...
            className="mb-8 flex h-20 w-20 items-center justify-center border border-white/10 bg-white/[0.03]"
          >
            {isBusy ? (
              <Sparkles className="h-8 w-8 stroke-[1.25] text-white" />
            ) : isDragActive ? (
              <Sparkles className="h-8 w-8 stroke-[1.25] text-white" />
            ) : (
              <Upload className="h-8 w-8 stroke-[1.25] text-white" />
            )}
          </motion.div>

          <p className="text-[0.65rem] uppercase tracking-[0.45em] text-zinc-500">Magic Upload</p>
          <h1 className="mt-4 max-w-3xl text-4xl font-black tracking-[-0.05em] text-white sm:text-5xl lg:text-6xl">
            {isBusy ? "Converting your document." : "Drop a document to start the split."}
          </h1>
          <p className="mt-5 max-w-2xl text-sm font-light leading-7 text-zinc-400 sm:text-base">
            {isBusy
              ? "The uploaded file is being parsed, cleaned up, and prepared for the split workspace."
              : "PDF and Markdown files land here, then the card morphs into a premium split workspace for preview and live output."}
          </p>

          <div className="mt-10 flex flex-wrap items-center justify-center gap-3 text-xs uppercase tracking-[0.28em] text-zinc-500">
          ...
          <p className={cn("mt-8 text-sm text-zinc-500", errorMessage ? "text-red-300" : "")}>{errorMessage ?? (isBusy ? "Parsing content and preparing the AI sidebar." : "Subtle border shimmer activates while a file is dragged over the zone.")}</p>
        </div>
      </button>
